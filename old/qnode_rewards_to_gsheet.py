...
# This version uses a different method to find the next empty cell, it takes MUCH MORE read operations but less server memory
...
def find_next_empty_row(sheet, column, start_row):
    # Fetch all relevant cells in the column from the starting row to the bottom
    cell_range = f"{column}{start_row}:{column}{sheet.row_count}"
    cells = sheet.range(cell_range)

    # Iterate through the fetched cells to find the first empty one
    for cell in cells:
        if not cell.value:
            return cell.row
    return None
